...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.БлокируемыеОбъекты   = "Документ.ЗаказПокупателя, Отчет._ДемоСтатусыЗаказовПокупателей";
...
// Проверяет, что обработчик отложенного обновления обработал документ ЗаказПокупателя.
//
// Параметры:
...
Функция ЗаполненыДанныеДляСпискаЗаказПокупателя(Параметры) Экспорт
	...
	Если ТипЗнч(Параметры.Данные) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
...
Процедура ВосстановитьДвиженияДокументов(Параметры) Экспорт
	...
	Запрос = Новый Запрос(" 
		...
		|		Документ.ПоступлениеТоваров.Товары КАК ИсточникДвижений
		|	ПО 
		|		ИсточникДвижений.Ссылка = Документы.Ссылка
		|	ЛЕВОЕ СОЕДИНЕНИЕ
		|		РегистрБухгалтерии._ДемоОсновной КАК Движения
		|	ПО 
		|		Движения.Регистратор ССЫЛКА Документ.ПоступлениеТоваров
		...
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
		|		Документ.РеализацияТоваров.Товары КАК ИсточникДвижений
		|	ПО 
		|		ИсточникДвижений.Ссылка = Документы.Ссылка
		|	ЛЕВОЕ СОЕДИНЕНИЕ
		|		РегистрБухгалтерии._ДемоОсновной КАК Движения
		|	ПО 
		|		Движения.Регистратор ССЫЛКА Документ.РеализацияТоваров
		|		И Документы.Ссылка = Движения.Регистратор
		|	ГДЕ
		|		Документы.Проведен
		|	СГРУППИРОВАТЬ ПО
...
Процедура ЗаполнитьПоследовательностьДвиженияТоваров(Параметры) Экспорт 
	...
	Запрос = Новый Запрос(" 
		...
		|			ГДЕ
		|				Заполняемое.Регистратор ССЫЛКА Документ.ПоступлениеТоваров
		...
		|				Последовательность._ДемоДвижениеТоваров КАК Заполняемое
		|			ПО
		|				ВЫРАЗИТЬ(Заполняемое.Регистратор КАК Документ.ПоступлениеТоваров) = Поступление.Ссылка
		|			ГДЕ
		...
		|				Последовательность._ДемоДвижениеТоваров КАК Заполняемое
		|			ГДЕ
		|				Заполняемое.Регистратор ССЫЛКА Документ.РеализацияТоваров
		|				И Заполняемое.Номенклатура = ЗНАЧЕНИЕ(Справочник._ДемоНоменклатура.ПустаяСсылка)
		|			
		|			ОБЪЕДИНИТЬ ВСЕ
		|			
		...
		|				Последовательность._ДемоДвижениеТоваров КАК Заполняемое
		|			ПО
		|				ВЫРАЗИТЬ(Заполняемое.Регистратор КАК Документ.РеализацияТоваров) = Реализация.Ссылка
		|			ГДЕ
		|				Заполняемое.Регистратор ЕСТЬ NULL
		|		)
...
Процедура ОбновитьКонтактнуюИнформациюЗаказовПокупателя(Параметры) Экспорт
	...
	Если Параметры.ПрогрессВыполнения.ВсегоОбъектов = 0 Тогда
		...
		|ГДЕ
		|	((ВЫРАЗИТЬ(ЗаказПокупателя.АдресДоставки КАК СТРОКА(100))) <> """"
		|				И ЗаказПокупателя.АдресДоставкиСтрокой = """"
		|			ИЛИ (ВЫРАЗИТЬ(ЗаказПокупателя.ЭлектроннаяПочта КАК СТРОКА(100))) <> """"
		|				И ЗаказПокупателя.ЭлектроннаяПочтаСтрокой = """")";
